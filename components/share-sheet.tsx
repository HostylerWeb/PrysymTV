...
function ShareIcon({ id }: { id: SharePlatform }) {
...
export function ShareSheet({ isOpen, onClose, title, url, targetId, onShared }: ShareSheetProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full md:max-w-md bg-background rounded-t-3xl md:rounded-2xl p-6 max-h-[85vh] overflow-y-auto"
      ...
      >
        ...
        <p className="text-sm text-muted-foreground line-clamp-2 mb-5">{title}</p>
        ...
        <div className="grid grid-cols-4 gap-3 mb-5">
          {links.map((link) => (
            ...
            >
              <span
                className="w-12 h-12 rounded-full flex items-center justify-center shadow-md transition-transform group-hover:scale-105"
                style={{ backgroundColor: link.color }}
              >
                <ShareIcon id={link.id} />
              </span>
