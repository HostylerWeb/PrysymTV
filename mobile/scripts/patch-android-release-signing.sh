#!/usr/bin/env bash
# Re-apply release signing block after `npx expo prebuild --platform android`.
...
if [[ ! -f "$GRADLE" ]]; then
  echo "Missing $GRADLE — run: npx expo prebuild --platform android" >&2
  exit 1
fi
...
echo "Done. Ensure android/keystore.properties and android/app/prysymtv-release.keystore exist."
