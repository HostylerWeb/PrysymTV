...
export function useOtaUpdates(): OtaUpdateState {
  ...
  useEffect(() => {
    ...
    const updateId =
      downloadedUpdate?.type === 'new' ? downloadedUpdate.updateId : 'pending';
    if (!updateId || notifiedUpdateIdRef.current === updateId) return;
    notifiedUpdateIdRef.current = updateId;
    void Notifications.scheduleNotificationAsync({
      content: {
        title: 'Update ready',
        body: 'Restart PrysymTV to load the latest version.',
