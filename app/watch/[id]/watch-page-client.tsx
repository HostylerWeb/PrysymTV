...
import {
  ...
  Bookmark, ChevronDown, ChevronUp, Send, Lock, Flag, ListMusic,
  Play, Gift,
} from "lucide-react"
...
function WatchPageContent({ videoId }: { videoId: string }) {
  const id = videoId
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setReplyingTo(null)
      try {
        const [api, feed, commentsRes] = await Promise.all([
          fetchVideo(id),
          fetchVideosBrowse({ page: 1, limit: 12, mode: "videos" }).catch(() => null),
          fetchVideoComments(id, 1).catch(() => null),
        ])
        if (cancelled) return
        if (api.type === "short") {
        ...
        if (commentsRes) {
          setComments(
            commentsRes.items.map((item) =>
              normalizeVideoComment(item as unknown as Record<string, unknown>),
            ),
          )
          setCommentsTotal(commentsRes.meta.total)
        }
      } catch {
  ...
  }, [id, router, highlightCommentId, openCommentsFromUrl])
